...
#define VENDOR_VER_STR "ro.vendor.build.date.utc="
#define VENDOR_VER_STR_LEN 25
...
static int get_version(char *ver_str, size_t len, char *PART_PATH, char *VER_STR,
        size_t VER_STR_LEN) {
...
Value* VerifyModemFn(const char* name, State* state,
    ...
    ret = get_version(current_modem_version, MODEM_VER_BUF_LEN, MODEM_PART_PATH,
            MODEM_VER_STR, MODEM_VER_STR_LEN);
    if (ret) {
...
/* verify_vendor("VENDOR_VERSION", "VNDK_VERSION") */
Value* VerifyVendorFn(const char* name, State* state,
    ...
    ret = get_version(current_vendor_version, VENDOR_VER_BUF_LEN, VENDOR_PART_PATH,
            VENDOR_VER_STR, VENDOR_VER_STR_LEN);
    if (ret) {
        return ErrorAbort(state, kVendorFailure,
    ...
    // Return 0 now if build is older than required
    if (!ret) {
        return StringValue(strdup("0"));
    }

    // Check for VNDK version
    ret = get_version(current_vndk_version, VNDK_VER_BUF_LEN, VENDOR_PART_PATH,
            VNDK_VER_STR, VNDK_VER_STR_LEN);
    if (ret) {
        return ErrorAbort(state, kVendorFailure,
