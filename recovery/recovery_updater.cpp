...
static int get_version(char *ver_str, size_t len,
        char *lookup_part_path, char *lookup_str, size_t lookup_str_len) {
    ...
    fd = open(lookup_part_path, O_RDONLY);
    if (fd < 0) {
        ret = errno;
        goto err_ret;
    }
    ...
    if (offset != NULL) {
        snprintf(ver_str, len, "%s", offset + lookup_str_len);
    } else {
        ret = -ENOENT;
    }
