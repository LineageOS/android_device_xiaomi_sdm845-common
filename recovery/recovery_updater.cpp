...
#define VENDOR_VER_STR "o.vendor.build.date.utc="
#define VENDOR_VER_STR_LEN 24
#define VENDOR_VER_BUF_LEN 11
#define VNDK_VER_STR "o.vndk.version="
#define VNDK_VER_STR_LEN 15
...
static int get_version(char *ver_str, size_t len, char *PART_PATH, char *VER_STR, size_t VER_STR_LEN) {
    int ret = 0;
    int fd;
    int size;
    char *data = NULL;
    ...
    offset = bm_search(data, size, VER_STR, VER_STR_LEN);
    if (offset != NULL) {
        snprintf(ver_str, len, "%s", offset + VER_STR_LEN);
    } else {
        ret = -ENOENT;
    }
...
Value* VerifyModemFn(const char* name, State* state,
    ...
    ret = get_version(current_modem_version, MODEM_VER_BUF_LEN, MODEM_PART_PATH, MODEM_VER_STR, MODEM_VER_STR_LEN);
    if (ret) {
...
/* verify_vendor("VENDOR_VERSION", "VENDOR_VERSION", ...) */
Value* VerifyVendorFn(const char* name, State* state,
                     const std::vector<std::unique_ptr<Expr>>& argv) {
    char current_vendor_version[VENDOR_VER_BUF_LEN];
    char current_vndk_version[VNDK_VER_BUF_LEN];
    int ret;

    // Check for build date
    ret = get_version(current_vendor_version, VENDOR_VER_BUF_LEN, VENDOR_PART_PATH, VENDOR_VER_STR, VENDOR_VER_STR_LEN);
    if (ret) {
        return ErrorAbort(state, kVendorFailure,
                "%s() failed to read current vendor build date: %d", name, ret);
    }
    ...
    ret = 0;
    if (std::stoi(current_vendor_version) >= std::stoi(args[0])) {
            ret = 1;
    }

    // Return now if build date didn't match
    if (!ret) {
        return StringValue(strdup("0"));
    }

    // Check for VNDK version
    ret = get_version(current_vndk_version, VNDK_VER_BUF_LEN, VENDOR_PART_PATH, VNDK_VER_STR, VNDK_VER_STR_LEN);
    if (ret) {
        return ErrorAbort(state, kVendorFailure,
