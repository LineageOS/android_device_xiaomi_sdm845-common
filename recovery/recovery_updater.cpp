...
static int get_info(char* ver_str, size_t len,
                    char* lookup_str, size_t lookup_str_len, char* lookup_part_path) {
    int ret = 0;
    int fd;
    size_t size;
    char* data = NULL;
...
Value* VerifyModemFn(const char* name, State* state,
    ...
    ret = get_info(current_modem_version, MODEM_VER_BUF_LEN,
                   MODEM_VER_STR, MODEM_VER_STR_LEN, MODEM_PART_PATH);
    if (ret) {
...
Value* VerifyVendorFn(const char* name, State* state,
    ...
    ret = get_info(current_vendor_date, VENDOR_DATE_BUF_LEN,
                   VENDOR_DATE_STR, VENDOR_DATE_STR_LEN, VENDOR_PART_PATH);
    if (ret) {
        return ErrorAbort(state, kVendorFailure,
    ...
    ret = get_info(current_vndk_version, VNDK_VER_BUF_LEN,
                   VNDK_VER_STR, VNDK_VER_STR_LEN, VENDOR_PART_PATH);
    if (ret) {
        return ErrorAbort(state, kVendorFailure,
