...
def AddVendorAssertion(info, input_zip):
  ...
  # We call mount instead of using edify's mount() so that we can mount the partition as read-only
  mount_cmd='run_program("/sbin/mount", "/vendor", "-r");'
  info.script.AppendExtra(mount_cmd);
  android_info = info.input_zip.read("OTA/android-info.txt")
  ...
  if v and miui_version:
    ...
    firmware_version = miui_version.group(1).rstrip()
    # greater_than_int doesn't seem to check for equality - so decrement the date by 1
    build_date_utc = int(build_date_utc) - 1
    check_vndk = 'file_getprop("/vendor/default.prop","ro.vndk.version") == "{}"'
    check_build_date = 'greater_than_int(file_getprop("/vendor/build.prop", "ro.vendor.build.date.utc"), "{}")'
    abort_error = 'abort("ERROR: This package requires vendor from MIUI {} developer build or newer. Please upgrade vendor image along with matching firmware and retry!");'
    info.script.AppendExtra('assert(' + check_vndk.format(vndk_version) + ' && ' + check_build_date.format(build_date_utc) + ' || ' + abort_error.format(firmware_version) + ');');
  return
