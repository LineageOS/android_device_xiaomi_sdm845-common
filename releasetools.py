...
def AddVendorAssertion(info, input_zip):
  ...
  android_info = info.input_zip.read("OTA/android-info.txt")
  m = re.search(r'require\s+version-vendor\s*=\s*(.+)', android_info)
  miui_version = re.search(r'require\s+version-miui\s*=\s*(.+)', android_info)
  if m and miui_version:
    build_date_utc, vndk_version = m.group(1).rstrip().split(',')
    firmware_version = miui_version.group(1).rstrip
    ...
    check_builddate='greater_than_int(file_getprop("/vendor/build.prop", "ro.vendor.build.date.utc"), "{}")'
